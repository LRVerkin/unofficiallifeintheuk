...
const specialRuleSchema = z.discriminatedUnion("kind", [
  z.object({
    kind: z.literal("must_include"),
    optionIds: z.array(z.number().int().positive()).nonempty(),
  }),
  z.object({
    kind: z.literal("fail_on_hint"),
    hintId: z.string().min(1),
  }),
  z.object({
    kind: z.literal("auto_pass"),
...
const baseQuestionSchema = z.object({
  ...
  credits: z.array(z.string().min(1)).default([]),
});
