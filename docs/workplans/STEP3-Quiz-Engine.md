...
- [x] Design TypeScript interfaces for quiz session state, actions, reducers, and events.
- [x] Implement `lib/quiz/session.ts` with functions: `createSession`, `answerQuestion`, `nextQuestion`, `previousQuestion`, `completeSession`, `resetSession`.
- [x] Build `lib/quiz/sampler.ts` supporting deterministic sampling (seed input) and question ordering logic with tests verifying reproducibility.
- [x] Implement `lib/quiz/scoring.ts` covering single, multiple, rank, and text question scoring (current dataset uses baseline rules; future structured special rules can extend this).
- [x] Create `lib/quiz/validators.ts` enforcing completion requirements (multi-select minimums, rank completeness, text normalization).
- [x] Implement persona resolution in `lib/personas.ts`, mapping percentage ranges to persona metadata + diploma assets; handle edge cases (rounding, boundaries).
- [x] Add analytics helper stubs (`lib/analytics.ts`) capturing event names/payload types; add `lib/seo.ts` placeholders for share metadata.
- [x] Write Vitest unit tests covering every module, including failure modes (invalid state transitions, invalid answers, persona gaps).
- [x] Generate coverage report ensuring ≥90% coverage for the quiz engine namespace (checked via `pnpm test:unit`; additional per-file coverage can be captured later if needed).
...
- Dataset currently stores `specialRules` as descriptive strings. Engine exposes string arrays so future structured rules (`allowPartial`, `failOnHint`, etc.) can be added without breaking consumers.
- Text normalization defaults to lower-case + trim; fuzzy matching is opt-in per question (Q006). Document any new strategies when adding more text questions.
